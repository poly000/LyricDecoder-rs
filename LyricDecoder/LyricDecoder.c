...
const unsigned char key[] = {64, 71, 97, 119, 94,  50,  116, 71,
                             81, 54, 49, 45,  206, 210, 110, 105};

// unsigned char QQKey[] = "!@#)(*$%123ZXC!@!@#)(NHL";
const unsigned char QQKey[] = {0x21, 0x40, 0x23, 0x29, 0x28, 0x2A, 0x24, 0x25,
                               0x31, 0x32, 0x33, 0x5A, 0x58, 0x43, 0x21, 0x40,
                               0x21, 0x40, 0x23, 0x29, 0x28, 0x4E, 0x48, 0x4C};

// Deflate from the src buffer. The returned memory is allocated using malloc,
// and the caller is responsible for freeing it. A trailing '\0' is added to the
// decompress result Returns NULL on failure.
...
char *krcdecode(char *src, int src_len) {
    if (src_len < 4 || memcmp(src, "krc1", 4) != 0) return NULL;
...
char *qrcdecode(char *src, int src_len) {
    ...
    for (int i = 0; i < src_len; i += 8)
        three_des_crypt((BYTE *)src + i, (BYTE *)src + i, schedule);

    return (char *)deflate_memory((Bytef *)src, src_len);
}
